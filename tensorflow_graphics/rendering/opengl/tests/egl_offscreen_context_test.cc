...
TEST(EglOffscreenContextTest, TestCreate) {
  ...
  EXPECT_TRUE(context.Create(800, 600));
}

TEST(EglOffscreenContextTest, TestMakeCurrentWorks) {
  EGLOffscreenContext context1;
  EGLOffscreenContext context2;

  EXPECT_TRUE(context1.Create(800, 600));
  EXPECT_TRUE(context2.Create(400, 300));
  EXPECT_TRUE(context1.MakeCurrent());
  EXPECT_TRUE(context2.MakeCurrent());
}

TEST(EglOffscreenContextTest, TestMakeCurrentFails) {
  EGLOffscreenContext context;
