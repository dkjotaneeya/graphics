...
bool EGLOffscreenContext::Create(const int pixel_buffer_width,
  ...
  auto initialize_cleanup = tensorflow::gtl::MakeCleanup(
      [this]() { TerminateInitializedEGLDisplay(this->display_); });
  ...
  auto surface_cleanup = tensorflow::gtl::MakeCleanup([this]() {
    eglDestroySurface(this->display_, this->pixel_buffer_surface_);
  });
