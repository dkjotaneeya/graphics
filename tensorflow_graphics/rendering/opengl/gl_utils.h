...
class Program {
 public:
  ...
  // shader is composed of a string containing the code to compile, and a GLenum
  // defining the type of the shader which must be one of GL_COMPUTE_SHADER,
...
class RenderTargets {
 public:
  ...
  // Creates creates a depth render buffer and a color render buffer. After
  // creation, these two render buffer are attached to the frame buffer.
  //
  // Arguments:
  ...
  //   A boolean set to false if any error occured during the process, and set
  //   to true otherwise.
  bool ReadPixels(std::vector<T>* buffer) const;
 ...
 private:
  ...
  static bool CreateValidInternalFormat(
      ...
      std::unique_ptr<RenderTargets<T>>* render_targets);
  bool ReadPixelsValidPixelType(std::vector<T>* buffer,
...
template <typename T>
bool RenderTargets<T>::ReadPixels(std::vector<T>* buffer) const {
  std::cerr << "Unsupported type " << typeid(T).name() << std::endl;
  return false;
}

template <>
inline bool RenderTargets<float>::ReadPixels(std::vector<float>* buffer) const {
  return ReadPixelsValidPixelType(buffer, GL_FLOAT);
}

template <>
inline bool RenderTargets<uint8>::ReadPixels(std::vector<uint8>* buffer) const {
  return ReadPixelsValidPixelType(buffer, GL_UNSIGNED_BYTE);
}

template <typename T>
bool RenderTargets<T>::ReadPixelsValidPixelType(std::vector<T>* buffer,
                                                GLenum pixel_type) const {
...
class ShaderStorageBuffer {
 public:
  ...
  template <typename T>
  bool Upload(const std::vector<T>& data);
...
template <typename T>
bool ShaderStorageBuffer::Upload(const std::vector<T>& data) {
